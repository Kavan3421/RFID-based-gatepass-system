...
export async function GET(req) {
  try {
    ...
    if (!user || !user.rfid_tag) {
      return NextResponse.json(
        { success: false, message: "Unauthorized user or missing RFID tag." },
        { status: 401 }
      );
    }
    ...
    const currentRfidTag = user.rfid_tag;
    ...
    console.error("Error in User databydate API:", err);
